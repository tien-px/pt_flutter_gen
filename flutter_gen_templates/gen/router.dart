...
class AppRouter {
  ...
  Future? to{{ item.class_name }}({int? id}) {
    return Get.toNamed(Routes.{{ item.route_name }}, arguments: {{ item.class_name }}Args(), id: id);
  }

  Future? offAll{{ item.class_name }}({int? id}) {
    return Get.offAllNamed(Routes.{{ item.route_name }}, arguments: {{ item.class_name }}Args(), id: id);
  }

  Future? offAndTo{{ item.class_name }}({int? id}) {
    return Get.offAndToNamed(Routes.{{ item.route_name }}, arguments: {{ item.class_name }}Args(), id: id);
  }

  Future? off{{ item.class_name }}({int? id}) {
    return Get.offNamed(Routes.{{ item.route_name }}, arguments: {{ item.class_name }}Args(), id: id);
  }

  Future? offUntil{{ item.class_name }}({int? id}) {
    return Get.offNamedUntil(Routes.{{ item.route_name }}, (_) => false, arguments: {{ item.class_name }}Args());
  }
