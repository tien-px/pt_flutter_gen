...
def _update_directory(
    operation, path, recursive=False, line_by_line=False, extension=""
):
    try:
        files_updated = []
        for name in sorted(os.listdir(path)):
            ...
            results = update(operation, full_path, recursive, line_by_line, extension)
...
def update(operation, path, recursive=False, line_by_line=False, extension=""):
    ...
    if os.path.isfile(path):
        ...
        return _update_directory(operation, path, recursive, line_by_line, extension)
    else:
        assert False, "Path is neither file nor directory."


def replace(pattern, replacement, path, recursive=False, regex=False, extension=""):
    if regex:
